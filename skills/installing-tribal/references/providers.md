...
- Inference (cloud): a current mini-tier model such as `gpt-5.4-mini` across the three stages is a low-cost, capable choice.
- Anthropic serves the inference stages only; it has no embedding API, so embedding stays on Ollama or OpenAI.
